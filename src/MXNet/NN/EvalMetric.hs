...
class EvalMetricMethod metric where
    evaluate :: (DType dtype, MonadIO m) 
             => metric dtype                        -- evaluation metric
             -> M.HashMap String (A.NDArray dtype)  -- network bindings
             -> [A.NDArray dtype]                   -- output of the network
             -> TrainM dtype m ()
    format   :: (DType dtype, MonadIO m) => metric dtype -> TrainM dtype m String
